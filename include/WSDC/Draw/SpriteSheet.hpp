...
SpriteSheet& SpriteSheet::offset(const int& px_w, const int& px_h) noexcept {
    _offset.w = px_w;
    _offset.h = px_h;
    return *this;
}

SpriteSheet& SpriteSheet::separator(const int& px_w, const int& px_h) noexcept {
    _separation.w = px_w;
    _separation.h = px_h;
    return *this;
}

SpriteSheet& SpriteSheet::size(const int& px_w, const int& px_h) noexcept {
    _sprite_size.w = px_w;
    _sprite_size.h = px_h;
    return *this;
}

SpriteSheet& SpriteSheet::load(const char* filepath) {
    _sprite_sheet.load(filepath);
    return *this;
}

std::optional<WSDC::Draw::Image> SpriteSheet::operator()(const int& index_x, const int& index_y) const {
    Geo::Rect<int> rect;
